...
import { AddItemStuff } from "@/app/dashboard/addItemStuff";
...
export default async function RecipesPage(props: {
	...
	const calculateTotalEnergy = (mealItems: typeof items) => {
		return mealItems.reduce(
			(sum, item) => sum + calculateEnergy(item.quantity, item.food.energy),
			0,
		);
	};
	...
	const calculateTotalProtein = (mealItems: typeof items) => {
		return mealItems.reduce(
			(sum, item) => sum + calculateProtein(item.quantity, item.food.protein),
			0,
		);
	};
